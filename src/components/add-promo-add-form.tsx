...
export function AddPromoAdForm({
  setOpen,
}: {
  setOpen: (open: boolean) => void;
}) {
  const { toast } = useToast();
  const { addPromoAd } = useDataContext();
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
        ...
        ADDSUBTITLE: '',
        ADDADDRESS: '',
        IMAGEADDRESS: 'https://placehold.co/600x400.png',
        ORDER: 0,
    },
  });
  ...
  async function onSubmit(data: FormValues) {
    setIsLoading(true);
    try {
      await addPromoAd(data);
      toast({
        title: 'Ad Created',
        description: 'The promotional ad has been successfully created.',
      });
      setOpen(false);
    } catch (error) {
      toast({
        ...
        description: 'An error occurred while trying to create the ad.',
      });
    } finally {
      setIsLoading(false);
    }
